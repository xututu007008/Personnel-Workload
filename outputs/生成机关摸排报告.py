...
def load_workload():
    return latest_person_rows(read_sheet1(latest("职能部门工作量摸排表", ".xlsx")), "姓名", "部门", "填写日期")
...
def load_employee_base() -> pd.DataFrame:
    ...
    base = roster[
        roster["部门"].isin([d for d in SCOPE_DEPTS if d != "南京永利重工制造有限公司"])
        & ~roster["职位"].str.contains(MIDDLE_RE, na=False)
    ][["部门", "姓名"]].drop_duplicates("姓名")
    base = base[~((base["部门"] == "市场开发部") & (base["姓名"] == "闫至琴"))]
    base = pd.concat([base, pd.DataFrame([{"部门": "综合办公室", "姓名": "张立"}])], ignore_index=True)
    base = pd.concat(
        [base, pd.DataFrame([{"部门": "南京永利重工制造有限公司", "姓名": n} for n in NANJING_STAFF])],
        ignore_index=True,
    )
...
def build_erp_report(supervisor):
    ...
    conclusion = [
        f"`资产财务部` 仍是近三年ERP流程最集中的部门，参与流程 `{fmt(person.loc[person['部门'].eq('资产财务部'), '参与审批者'].sum(), 0)}` 次，承担主要审核压力。",
        f"`综合办公室`、`技术中心`、`市场开发部` 无ERP痕迹人数较多，分别为 `{int((no_ops['部门']=='综合办公室').sum())}`、`{int((no_ops['部门']=='技术中心').sum())}`、`{int((no_ops['部门']=='市场开发部').sum())}` 人。",
        f"`南京永利重工制造有限公司` 按旧稿口径补入 `9` 人，其中 `{int(person[person['部门'].eq('南京永利重工制造有限公司')]['有ERP痕迹'].sum())}` 人已有近三年ERP操作记录，`{int((no_ops['部门']=='南京永利重工制造有限公司').sum())}` 人仍无操作痕迹。",
...
- ERP `近三年` 原始操作表中有实际流程量的人员共 `{len(raw_names)}` 人，本报告仅纳入机关员工 `127` 人口径内人员。
...
def build_cert_report(supervisor):
    ...
    for _, item in roster.iterrows():
        ...
        if not name or name in roster_start:
            continue
    ...
    for r in supervisor:
        ...
        honor = merge_cert(yearend_item.get("荣誉证书"), r.get("荣誉证书"))
    ...
    detail = [[r["部门"], r["姓名"], fmt(r["工作年限"], 1), clean(r.get("规则重算评级")), SALARY.get(clean(r.get("规则重算评级")), ""), clean(r.get("主管主观评级")), SALARY.get(clean(r.get("主管主观评级")), ""), r["职称证书"], r["资格证书"], r["荣誉证书"]] for r in rows_sorted]
    review_rows = [[i, name, cert] for i, (name, cert) in enumerate(review.items(), 1)]
...
本次分析以当前已经纳入主管评价的 `{len(rows)}` 名非中层员工为基础。员工 `按20题计算等级` 统一采用 `机关员工主管评价20题重算报告-{DATE}.md` 中的规则重算等级；证书信息优先取当前文件夹 `年终总结` 开头文件中的 `有效职称证书`、`有效资格证书`、`其他荣誉证书` 字段，并结合主管评价表中的证书字段合并去重，最后按人力资源提供的 `人员证书复核.png` 对原无证书人员进行补充核验。
...
{table(["部门", "姓名", "工作年限", "按20题计算等级", "按20题计算等级对应薪资中位数", "主管评价等级", "主管评价等级对应薪资中位数", "职称证书", "资格证书", "荣誉证书"], [[r["部门"], r["姓名"], fmt(r["工作年限"], 1), clean(r.get("规则重算评级")), SALARY.get(clean(r.get("规则重算评级")), ""), clean(r.get("主管主观评级")), SALARY.get(clean(r.get("主管主观评级")), ""), r["职称证书"], r["资格证书"], r["荣誉证书"]] for r in unmatched])}
...
def build_workload_overview(workload, supervisor):
    ...
    for r in supervisor:
        ...
        salary_grade = clean(r.get("主管评级对应公司等级"))
        if sup_grade in GRADE_ORDER and salary_grade in GRADE_ORDER:
            cmp_value = grade_idx(sup_grade) - grade_idx(salary_grade)
            relation = "一致" if cmp_value == 0 else ("主管评级低于薪酬区间等级" if cmp_value > 0 else "主管评级高于薪酬区间等级")
            comparable.append((r, relation))
            if relation != "一致":
                salary_rows.append([clean(r.get("成员部门")), clean(r.get("成员姓名")), sup_grade, salary_grade, relation, clean(r.get("薪资区间-最小值（绩效考核最低值）")), clean(r.get("薪资区间-最大值（绩效考核最高值）"))])
...
1. 自动工作负荷中 `超负荷/满负荷` 合计 `{workload_load_counts.get("超负荷", 0) + workload_load_counts.get("满负荷", 0)}` 人，说明机关岗位仍存在较明显承压面。
...
{table(["姓名", "当前部门", "自动生成评级", "主管主观评级"], [[clean(r.get("成员姓名")), clean(r.get("成员部门")), clean(r.get("自动生成评级")), clean(r.get("主管主观评级"))] for r in auto_sup_diff])}
...
{table(["姓名", "当前部门", "按20题计算等级", "主管主观评级"], [[clean(r.get("成员姓名")), clean(r.get("成员部门")), clean(r.get("规则重算评级")), clean(r.get("主管主观评级"))] for r in rule_sup_diff])}
...
- 已形成薪酬区间等级、可直接比较：`{len(comparable)}` 人
- 未形成薪酬区间等级、暂无法比较：`{len(supervisor) - len(comparable)}` 人
- `主观评级与薪酬区间等级一致`：`{salary_relation_counts.get("一致", 0)}`
- `主观评级高于薪酬区间等级`：`{salary_relation_counts.get("主管评级高于薪酬区间等级", 0)}`
- `主观评级低于薪酬区间等级`：`{salary_relation_counts.get("主管评级低于薪酬区间等级", 0)}`

### 2. 不一致人员名单

{table(["部门", "姓名", "主管主观评级", "薪酬区间等级", "差异判断", "薪资区间最小值", "薪资区间最大值"], salary_rows)}

### 3. 本维度结论

1. 当前可比较样本中，一致样本 `{salary_relation_counts.get("一致", 0)}` 人。
2. 薪酬区间未形成有效等级的样本仍有 `{len(supervisor) - len(comparable)}` 人，后续需要补齐后再做最终判断。
...
{table(["姓名", "当前部门", "岗位", "主管主观评级", "岗位必要性", "离职影响"], role_contrast_rows(weak_necessity))}

### 3. 离职影响偏弱但评级不低样本

{table(["姓名", "当前部门", "岗位", "主管主观评级", "岗位必要性", "离职影响"], role_contrast_rows(weak_leave))}
...
{table(["姓名", "当前部门", "自动生成评级", "主管主观评级", "最终评价级别", "异常项数", "异常选项"], abnormal_rows(b_plus_abnormal))}

### 2. C类且异常更多人员

{table(["姓名", "当前部门", "自动生成评级", "主管主观评级", "最终评价级别", "异常项数", "异常选项"], abnormal_rows(c_abnormal))}
...
{table(["姓名", "当前部门", "最终评价级别", "自动生成评级", "主管主观评级", "员工自评总体工作负荷", "自动工作负荷", "低负荷信号"], low_load_rows)}
...
当前共有 `{len(idea_section_rows["员工认为最应取消、合并或简化的工作"])}` 人提出了有效想法，主要集中在：
...
当前共有 `{len(idea_section_rows["员工认为最值得推动但尚未落地的优化建议"])}` 人提出了有效想法，主要方向包括：
...
对问卷本身提出明确“不妥”意见的共有 `{len(idea_section_rows["员工认为问卷中存在的不妥情况"])}` 人，主要意见包括：
...
对问卷提出明确优化建议的共有 `{len(idea_section_rows["员工对问卷需要优化的问题和建议"])}` 人，主要集中在补充选项、优化题目表述、增加电子签名上传等方向。
...
当前直接表达出明确意向的人数为 `{len(mom_rows)}` 人，其中 `A.有强烈意向` `{mom_counter.get("A.有强烈意向", 0)}` 人，`B.有意向，但是不迫切，需考虑` `{mom_counter.get("B.有意向，但是不迫切，需考虑", 0)}` 人。更多人员为未填写或无意向，说明这类政策问题在现阶段仍偏观察项。
...
本轮摸排样本较旧稿明显扩展：工作量摸排 `{len(workload_records)}` 人，主管评价 `{len(supervisor)}` 人。自动工作负荷中 `超负荷` `{c_load.get('超负荷',0)}` 人、`满负荷` `{c_load.get('满负荷',0)}` 人，说明机关岗位仍存在较强承压面；同时也存在 `欠饱和/非饱和` 样本，需要结合岗位职责和ERP实际承担度复核。
...
def build_integrated(paths, workload_records, supervisor):
    ...
    for _, item in roster.iterrows():
        name = clean(item.get("姓名"))
        if name and name not in roster_start:
    ...
    for r in supervisor:
        ...
        try:
            ...
            years = 0
        cert_rows.append({**r, "姓名": name, "部门": clean(r.get("成员部门")), "工作年限": years, "原始有证书": original_has_any, "有证书": any([title, qual, honor])})
    ...
    review_rows = [[i, name, "原无证书人员复核", result] for i, (name, result) in enumerate(hr_review.items(), start=1)]
    reviewed_has_cert = [r for r in cert_rows if clean(r.get("姓名")) in hr_review and r["有证书"]]
    reviewed_no_cert = [r for r in cert_rows if clean(r.get("姓名")) in hr_review and not r["有证书"]]
...
当前机关部门及 `南京永利重工制造有限公司` 范围内，共涉及 `13` 个部门、`36` 名中层干部、`{TOTAL_STAFF}` 名机关员工。
...
在上述原始信息采集口径基础上，根据人力资源反馈的《人员证书复核》截图，本轮对原“无任何证书人员”名单作进一步核验。复核涉及 `{len(hr_review)}` 人，其中核实仍无证书 `{len(reviewed_no_cert)}` 人，补充确认已有证书 `{len(reviewed_has_cert)}` 人。

复核后仍无任何证书人员包括：`{cert_list(no_cert)}`。

人力资源补充确认已有证书人员包括：`{"、".join(f"{clean(r.get('姓名'))}（{hr_review.get(clean(r.get('姓名')))}）" for r in reviewed_has_cert) or "无"}`。
...
1. 主管评价20题重算后，`S/A/B` 合计 `{sum(1 for r in supervisor if clean(r.get("规则重算评级")) in {"S", "A", "B"})}` 人，`C/D/E/F` 合计 `{sum(1 for r in supervisor if clean(r.get("规则重算评级")) in {"C", "D", "E", "F"})}` 人，整体评价结构较旧稿有新增样本补充。
    ...
    while i < len(lines):
        ...
        if line.startswith("|"):
            ...
            if rows:
                ...
                font_size = 8.2 if max_cols <= 5 else 7.0 if max_cols <= 8 else 5.8
